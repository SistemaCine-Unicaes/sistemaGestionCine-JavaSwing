...
public class MapaAsientosView extends javax.swing.JDialog {
    ...
    public MapaAsientosView(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        generarButacas(5, 10);// Cuadrícula de prueba
    }
    
    private void generarButacas(int filas, int columnas) {
    panelCuadricula.setLayout(new GridLayout(filas, columnas, 5, 5));
    
    for (int i = 0; i < (filas * columnas); i++) {
        JToggleButton btnAsiento = new JToggleButton("A" + (i + 1));
        btnAsiento.setBackground(Color.GREEN); // Libre por defecto
        
        btnAsiento.addActionListener(e -> {
            if (btnAsiento.isSelected()) {
                btnAsiento.setBackground(Color.RED); // Ocupado
            } else {
                btnAsiento.setBackground(Color.GREEN); // Libre
            }
        });
        
        panelCuadricula.add(btnAsiento);
    }
    ...
    private void initComponents() {
        ...
        panelCuadricula.setBorder(javax.swing.BorderFactory.createLineBorder(null));
